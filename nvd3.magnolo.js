...
nv.models.sunburst = function() {
    ...
    //var y = d3.scale.sqrt();
    var u = d3.scale.pow().exponent(1.3).domain([0, 1]).range([0, width/2]);
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            container = g.data(partition.nodes).enter().append('g');
            path = container.append("path")
                .attr("d", arc)
                .style("fill", function (d) {
                  console.log((d.children ? d : d.parent).color);
                            ...
                            arcText.transition().duration(750)
                              ...
                              .attr("transform", function(d) {
                                  ...
                                  var n = (d.name || "").split(" ").length > 1,
                                    ...
                                    r = e + (n ? -.5 : 0);
                                  return "rotate(" + r + ")translate(" + (y(d.y) + c) + ")rotate(" + (e > 90 ? -180 : 0) + ")"
                              })
                          }
                      });
                })
            ...
            //.attr('transform', function(d){return "rotate("+computeTextRotation(d)+")";})
                ...
                .attr("transform", function (d) {
          				var n = (d.name || "").split(" ").length > 1,
          					e = 180 * x(d.x + d.dx / 2) / Math.PI - 90,
          					r = e + (n ? -.5 : 0);
                    console.log(d.name, n, e, r);

          				return "rotate(" + r + ")translate(" + (y(d.y) + c) + ")rotate(" + (e > 90 ? -180 : 0) + ")"
          			})
                .style("fill", function (t) {
          				return colorCheck(d3.rgb(colorMap(t))) < 125 ? "#eee" : "#000"
          			})
              ...
              text.append("tspan").attr("x", 0).text(function (d) {
          			return d.depth ? d.name.split(" ")[0] : d.name
          		});
               text.append("tspan").attr("x", 0).attr("dy", "1em").text(function (d) {
             ...
             function colorCheck(d) {
               return .299 * d.r + .587 * d.g + .114 * d.b
             }
